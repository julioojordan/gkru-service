...
func (repository *dataWilayahRepositoryImpl) FindOne(ctx *fiber.Ctx, tx *sql.Tx) (entity.DataWilayah, error) {
	...
	if err != nil {
		return entity.DataWilayah{}, helper.CreateErrorMessage("Gagal mengeksekusi query", err)
	}
	defer result.Close()
	...
	if result.Next() {
		...
		if err != nil {
			return entity.DataWilayah{}, helper.CreateErrorMessage("Gagal untuk scan result", err)
		}
...
func (repository *dataWilayahRepositoryImpl) Add(ctx *fiber.Ctx, tx *sql.Tx) (entity.DataWilayah, error) {
	...
	if err != nil {
		return entity.DataWilayah{}, helper.CreateErrorMessage("Gagal memasukan data wilayah", err)
	}

	lastInsertId, err := result.LastInsertId()
	if err != nil {
		return entity.DataWilayah{}, helper.CreateErrorMessage("Gagal untuk retrieve last inserted ID", err)
	}
...
func (repository *dataWilayahRepositoryImpl) Update(ctx *fiber.Ctx, tx *sql.Tx) (entity.DataWilayah, error) {
	...
	if err != nil {
		return entity.DataWilayah{}, helper.CreateErrorMessage("Gagal untuk update data wilayah", err)
	}
...
func (repository *dataWilayahRepositoryImpl) DeleteOne(ctx *fiber.Ctx, tx *sql.Tx) (entity.IdInt, error) {
	...
	if totalLingkungan.Total != 0 {
		return entity.IdInt{}, fiber.NewError(fiber.StatusInternalServerError, "Gagal untuk delete data wilayah karena data wilayah masih digunakan oleh beberapa lingkungan")
	}
	...
	if totalKeluarga.Total != 0 {
		return entity.IdInt{}, fiber.NewError(fiber.StatusInternalServerError, "Gagal untuk delete data wilayah karena data wilayah masih digunakan oleh KK")
	}

	// Executing the update statement
	_, err = tx.Exec(sqlScript, idWilayah)
	if err != nil {
		return entity.IdInt{}, helper.CreateErrorMessage("Gagal untuk delete data wilayah", err)
	}
